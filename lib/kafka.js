...
function stream(topic, consumerId, options, offset, callback) {
  ...
  log.info({ consumerOptions, topic, consumerId, offset }, 'Creating consumer...');
  ...
  consumer.on('event.log', ({ message, fac, severity }) => {
    ...
    log.debug({ consumerId, message, fac, severity }, 'stream.event.log');
  });

  consumer.on('event.error', log.error.bind(null, { consumerId }, 'stream.event.error'));
  consumer.on('error', log.error.bind(null, { consumerId }, 'stream.error'));
  ...
  consumer.on('ready', () => {
    ...
    }, 'Stream consumer ready');

    const consumeForever = (err, message) => {
      if (err) {
        log.error({
          err,
          ...
          'err.code': err.code,
          'err.errno': err.errno,
          'err.isFatal': err.isFatal,
          'err.isRetriable': err.isRetriable,
          'err.isTxnRequiresAbort': err.isTxnRequiresAbort,
          assignments: consumer.assignments(),
          subscription: consumer.subscription(),
        }, 'Received consume error');

        if (options.onConsumeRetry) options.onConsumeRetry({ err });

        consumer.consume(consumeForever);
      } else {
        callback(message);
      }
    };
  ...
  consumer.on('disconnected', log.error.bind(null, { consumerId }, 'stream.disconnected'));
  consumer.on('event.throttle', log.error.bind(null, { consumerId }, 'stream.event.throttle'));
  consumer.on('warning', log.error.bind(null, { consumerId }, 'stream.warning'));
