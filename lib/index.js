...
function create(options) {
  ...
  return new KafkaCache({
    ...
    kafkaWrite: createKafkaWrite(options),
...
function createKafkaWrite({ valueEncoding, log, topic, kafkaHost, writeCompression, readOnly }) {
  ...
  if (valueEncoding !== 'json') {
    log.warn({ valueEncoding}, 'KafkaCache.put is not supported for selected valueEncoding. .put() will throw if called!');
    return () => { throw new Error('KafkaCache.put not supported as kafka writes are only implemented for json values yet!'); }
  }
